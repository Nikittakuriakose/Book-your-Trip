...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // if user exista nd paswor id correct
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError('The user belongs to this token no longer exits!', 401),
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user)
    return next(new AppError('The user with this email doesnt exist', 404));

  //generate the random encyrpted reset token and save it to db
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save(); //while dealing with passwords always use save not update it wont run the validators
